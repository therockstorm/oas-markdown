...
var argv = require("yargs")
  ...
  .describe(
    "headings",
    "Set the value of the `headingLevel` parameter in the header so Shins knows how many heading levels to show in the table of contents."
  )
  .default("headings", 2)
  .boolean("html")
  ...
  .describe(
    "outfile",
    "File to write the output markdown to. If left blank, output sent to stdout."
  )
  .boolean("raw")
  .alias("r", "raw")
  ...
  .describe(
    "search",
    "Set the value of the `search` parameter in the header so Markdown processors like Shins include search or not in their output."
  )
  ...
  try {
    const initOpToDescMap = async () => {
      ...
      for (const line of output.split("\n")) {
        if (!op && !line) continue;
        if (line.startsWith("# ")) {
          if (op && desc) opToDesc.set(op, desc);
          desc = "";
          op = line.replace("# ", "");
        } else desc += `${line}\n`;
      }
      opToDesc.set(op, desc);
      return opToDesc;
    };
    ...
    Object.keys(api.paths).forEach((p) => {
      if (api.paths[p]) {
        Object.keys(api.paths[p]).forEach((m) => {
          ...
          method.description = opToDesc.get(method.operationId);
        });
      }
    });
