    ...
    if (!p.startsWith("x-")) {
      for (var m in source.paths[p]) {
        if (
          ...
          var sMethodUniqueName = (method.operation.operationId
...
function getParameters(data) {
  ...
  data.parameters = data.parameters.filter(
    (p) => p.in !== "header" && p.in !== "path" && p.in !== "query"
  );
  ...
  for (let param of data.parameters) {
    ...
    if (pSchema && !param.safeType) {
      ...
      if (pSchema["x-widdershins-oldRef"]) {
        ...
        param.safeType = schemaName
      }
      if (param.refName) param.safeType = param.refName
    }
    if (pSchema) {
...
function convertInner(api, options) {
  return new Promise(function (resolve, reject) {
    ...
    if (typeof templates === "undefined") {
      templates = dot.process({
        path: path.join(__dirname, "..", "templates", "openapi3"),
      })
    }
